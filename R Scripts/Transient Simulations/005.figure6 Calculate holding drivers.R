## Investigate which climate driver is causing a decreasing baseline for both Demersal and Planktivorous fish 
...
e2ep_transient_baseline <- function(guilds_to_crash,hold){
  ...
  master <- list(Biomasses = list(),
                 hold = character(0)) #How are we going to save all of this?
  ...
  for (i in 1:(length(transient_years))) {
    ...
    if (hold == "Boundary") {
      My_boundary_data <- readRDS("../Objects/Barents_Sea/NM/Boundary measurements.rds") %>%   
        filter(Year %in% seq(2010,2020)) %>%    # Import data
...
baselines <- future_map(.x = hold,
                        ~ {
                          e2ep_transient_baseline(guilds_to_crash = guild_to_crash,
                                                  hold = .x)
                        },
                        .progress = T)

saveRDS(baselines,"../Objects/Experiments/Investigate Climate Effects/hold_climate_drivers_DF.RDS")

guild_to_crash <- c("Planktivorous_fish")
...
saveRDS(baselines,"../Objects/Experiments/Investigate Climate Effects/hold_climate_drivers_PF.RDS")
toc()