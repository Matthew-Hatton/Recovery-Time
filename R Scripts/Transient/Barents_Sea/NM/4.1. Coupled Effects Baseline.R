...
e2ep_transient_baseline <- function(guilds_to_crash){
  ...
  for (i in 1:(length(transient_years))) {
    ...
    # Change target guild
    model[["data"]][["fleet.model"]][["HRscale_vector_multiplier"]][positions] <- rep(1,length(positions)) # keep fishing at baseline
    ...
    model[["data"]][["initial.state"]][1:nrow(init_con)] <- e2ep_extract_start(model = model,results = results,
                                                                               csv.output = F)[,1]
    p(message = paste0("Calculating ",transient_years[i]))
...
saveRDS(baselines,paste0("../Objects/Experiments/Coupled Effects/Baseline__fishing_",guild_to_crash,".RDS"))
