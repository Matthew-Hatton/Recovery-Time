## Run transient dynamics for 2020 watching recovery times of a large range of HR's
...
e2ep_transient <- function(relax,guilds_to_crash,crash) { # Guilds will take a vector of names of guilds to crash
  ...
  for (i in 2:(length(transient_years))) {
    ...
    My_light <- readRDS("../Objects/Barents_Sea/NM/Air temp and light.rds") %>% 
      filter(Shore == "Combined" & Year %in% seq(transient_years[i]-10,transient_years[i])) %>%               # Limit to reference period and variable - light only goes to 2019, so if past that, hold it at 2019 values
    ...
    My_air_temp <- readRDS("../Objects/Barents_Sea/NM/Air temp and light.rds") %>% 
      filter(Shore %in% c("Inshore","Offshore") & Year %in% seq(transient_years[i]-10,transient_years[i])) %>% 
    ...
    My_H_Flows <- readRDS("../Objects/Barents_Sea/NM/H-Flows.rds") %>% 
      filter(Year %in% seq(transient_years[i]-10,transient_years[i])) %>%                                     # Limit to reference period
    ...
    My_ice <- readRDS("../Objects/Barents_Sea/NM/TS.rds") %>% 
      ...
      filter(Year %in% seq(transient_years[i]-10,transient_years[i])) %>%  # Filter down to just the target year
      group_by(Month,Shore) %>% 
...
relax_values <- 0
crash <- seq(0,5.6,0.2) # baseline, MSY, 2x MSY
guilds_to_crash <- "Demersal_fish"
...
results_list <- future_map(crash,
                           ...
                           .progress = F)
saveRDS(results_list,paste0("../Objects/Experiments/Crash/Paper/Recovery_Time_Road_To_Recovery.RDS"))
# 
