...
baseline_df <- data.frame(
  ...
  baseline = map_dbl(baseline[["Biomasses"]], ~ .x$Model_annual_mean[24])) %>% #extract DF biomass
  mutate(MSC = baseline * 0.8)

baseline_non_ss_df <- data.frame(
  year = transient_years[1:length(baseline_non_ss[["Biomasses"]])],
  baseline = map_dbl(baseline_non_ss[["Biomasses"]], ~ .x$Model_annual_mean[24])) %>% #extract DF biomass
  mutate(MSC = baseline * 0.8)
...
for (i in 1:length(all)) {
  ...
  for (k in 1:3) {
    ...
    df <- data.frame(
      ...
      Biomass = map_dbl(hrs, ~ .x$Model_annual_mean[24])) %>% #extract DF biomass
...
non_ss_biomass <- ggplot() +
  ...
  geom_ribbon(
    data = baseline_non_ss_df,
    aes(x = year, y = baseline, ymin = baseline - (baseline * 0.2), ymax = baseline + (baseline * 0.2)), alpha = 0.1) +
  facet_wrap(~ Crash_Year, ncol = 3, scales = "free_x", strip.position = "top") +
  labs(x = "Year", y = "Planktivorous Fish Biomass (mmN/m2)", color = "Harvest Rate") +
  scale_x_continuous(limits = c(2020,2099)) +
