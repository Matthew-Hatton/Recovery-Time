...
#### Crashing the system ####
e2ep_transient_interval <- function(relax,guilds_to_crash,crash,interval) { # Guilds will take a vector of names of guilds to crash
  ...
  for (j in 1:length(interval)) {
    ...
    ## Read in climate data for current year in interval
    ## ...
    ...
    for (i in 2:length(years)) { # start from year after crash ie. 2:...
      #### Chemistry ####
