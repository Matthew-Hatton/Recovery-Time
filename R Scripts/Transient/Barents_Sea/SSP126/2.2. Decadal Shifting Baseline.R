...
e2ep_transient_baseline <- function(hr_scale){
  ...
  model[["data"]][["fleet.model"]][["HRscale_vector_multiplier"]] <- rep(hr_scale,length(model[["data"]][["fleet.model"]][["HRscale_vector_multiplier"]]))
  ...
  saveRDS(master,paste0("../Objects/Shifting_Baseline_Decadal_",hr_scale,"_fishing.RDS"))
}

future_map(.x = c(0,1),.f = e2ep_transient_baseline.,progress = T)
toc()