...
all_data <- readRDS("../Objects/Shifting_Baseline_Decadal_0_fishing_Demersal_fish.RDS")
...
# all_data_1x <- readRDS("../Objects/Shifting_Baseline_Decadal_10_fishing_Demersal_fish.RDS")
...
# all_data_10x <- readRDS("../Objects/Shifting_Baseline_Decadal_10_fishing.RDS")
# biomasses_10x <- all_data_10x[["Biomasses"]]
...
# }
# 
# all_biomasses_10x <- data.frame(Model_annual_mean = NA,
#                                Units = NA,
#                                Description = NA,
#                                Year = NA)
# for (i in 1:length(biomasses)) {
...
#                     Units = NA,
#                     Description = NA)
#   tmp_join <- rbind(tmp,biomasses_10x[[i]])
#   tmp_join$Year <- transient_years[i]
#   all_biomasses_10x <- rbind(all_biomasses_10x,tmp_join)
# }
...
# 
# names(all_biomasses_10x)[3] <- "Guild"
# all_biomasses_10x <- all_biomasses_10x %>% filter(Guild %in% useful)
...
# no_crash_10x_smooth <- all_biomasses_10x
...
ggplot() +
  ...
  geom_line(data = no_crash_0x_smooth,aes(x = Year,y = Model_annual_mean),color = "black",alpha = 0.8,linetype = "dashed") +
  ...
  #geom_line(data = no_crash_10x_smooth,aes(x = Year,y = Model_annual_mean),color = "darkgreen",alpha = 0.8,linetype = "dashed") +
  labs(x = "Year",
       y = "Guild Biomass",
