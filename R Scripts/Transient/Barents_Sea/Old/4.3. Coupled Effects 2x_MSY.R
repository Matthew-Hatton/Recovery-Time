## Script to extract coupled effects of climate and fishing in the BASELINE
## Scripts are split into 3 due to complex calculation of MSY
...
e2ep_transient_baseline <- function(guilds_to_crash){
  ...
  NO3_boundary <- readRDS("../Objects/Barents_Sea/NM/River nitrate and ammonia.rds") %>% subset(select = c(Month,Nitrate))       
  ...
  master <- list(All_Results = list(),
                 ...
                 MSY = list()) #How are we going to save all of this?
  ...
  for (i in 1:(length(transient_years))) {
    ...
    ## calculate MSY for current year
    ycurve_res <- e2ep_run_ycurve(model,nyears = 20,HRvector = seq(2,5,0.2),selection = "DEMERSAL")
    ...
    # Change target guild
    model[["data"]][["fleet.model"]][["HRscale_vector_multiplier"]][positions] <- rep(2*MSY,length(positions)) # keep fishing at baseline
    ...
    master[["MSY"]][[paste0(transient_years[i])]] <- 2*MSY
    ...
    model[["data"]][["initial.state"]][1:nrow(init_con)] <- e2ep_extract_start(model = model,results = results,
                                                                               csv.output = F)[,1]
    p(message = paste0("Calculating ",transient_years[i]," for 2x MSY = ",2 * MSY))
...
baselines <- e2ep_transient_baseline(guilds_to_crash = guild_to_crash)
saveRDS(baselines,paste0("../Objects/Experiments/Coupled Effects/2x_MSY__fishing_",guild_to_crash,".RDS"))
