...
e2ep_transient_baseline <- function(guilds_to_crash){
  ...
  for (i in 1:(length(transient_years))) {
    ...
    # Change target guild
    model[["data"]][["fleet.model"]][["HRscale_vector_multiplier"]][positions] <- rep(MSY,length(positions)) # keep fishing at baseline
    ...
    master[["MSY"]][[paste0(transient_years[i])]] <- MSY
    ...
    model[["data"]][["initial.state"]][1:nrow(init_con)] <- e2ep_extract_start(model = model,results = results,
                                                                               csv.output = F)[,1]
    p(message = paste0("Calculating ",transient_years[i]," for MSY = ",MSY))
...
saveRDS(baselines,paste0("../Objects/Experiments/Coupled Effects/MSY__fishing_",guild_to_crash,".RDS"))
