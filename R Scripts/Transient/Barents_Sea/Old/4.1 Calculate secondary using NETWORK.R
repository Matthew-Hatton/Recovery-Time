...
example <- all[[1]][["Network_Indicators"]][["2020"]][["2021"]] %>% 
...
fci_index <- 63  # Replace this with the correct row number for FCIb

fci_df <- with_progress({
  p <- progressor(steps = length(all))
  
  imap_dfr(all, function(rep_list, rep_index) {
    p()
    
    imap_dfr(rep_list$Network_Indicators, function(year_list, crash_year) {
      imap_dfr(year_list, function(df, year) {
        data.frame(
          HR = crash_hr_values[rep_index],
          crash = as.numeric(crash_year),
          year = as.numeric(year),
          biomass = as.numeric(df$NetworkData[fci_index])
        )
      })
    })
  })
...
#   labs(x = "Release Year", y = "Recovery Time (Years)", color = "Harvest Rate",subtitle = "Bird Recovery") +
#   scale_y_continuous(expand = c(0, 0), limits = c(0, NA)) +  # y-axis starts at 0
#   theme_minimal(base_size = 14) +
#   theme(legend.position = "top") +
#   NULL
# ggsave(paste0("../Figures/Transient/Barents_Sea/NM/Draft 1/Figure 3 pt 2 BIRDS.png"),
...
ggplot(biomass_df, aes(x = year, y = biomass, color = as.character(HR))) +
  ...
  NULL
ggsave("../Figures/Transient/Barents_Sea/NM/Draft 1/Figure 2 BIRD OMNIVORY.png",
       dpi = 1200,width = 25,unit = "cm",bg = "white")
...
#     aes(x = year, y = baseline), linetype = "dashed", inherit.aes = FALSE,alpha = 0.6
#   ) +
#   # geom_line(explore, aes(x = year, y = biomass, color = as.character(HR)),linewidth = 1) +
#   labs(x = "Release Year", y = "Revised Finns Cycling Index", color = "Harvest Rate") +
#   scale_y_continuous(expand = c(0, 0), limits = c(0, NA)) +  # y-axis starts at 0
#   theme_minimal(base_size = 14) +
#   theme(legend.position = "top") +
#   NULL
